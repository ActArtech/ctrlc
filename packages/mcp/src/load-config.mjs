/**
 * Load a host SectionPackConfig (mirrors packages/cli/src/load-config.mjs).
 * Self-contained so @ctrlc/mcp does not depend on the full CLI package.
 */
    ...
    if (!tsxReady) {
      throw new Error(
        `Cannot load TypeScript config without tsx: ${absPath}\n` +
          `Install tsx (npm i -D tsx) or pass a .mjs/.js config via configPath.`,
      );
    }
  }
